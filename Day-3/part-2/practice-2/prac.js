...
const { recipes } = data;
const mainId = document.getElementById("root");
...
const createUI = () => {
  recipes.forEach((elements) => {
    ...
    newDiv.innerHTML = `
        <h2>${elements.name}</h2>
        <img src=${elements.image} alt = ${elements.name} width = "200">
        <p id = "rating">${elements.rating}</p>
        <p>${elements.cuisine}</p>
        <p>${elements.mealType}</p>
    `;
    mainId.append(newDiv);
  });
};
